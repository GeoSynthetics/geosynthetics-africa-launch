        ...
        <div className="container-page py-16 md:py-24 relative z-10 flex flex-col md:flex-row gap-10">
          <div className="flex-1">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-4">
              ...
              <Button
                asChild
                variant="outline"
                className="bg-transparent border-white/30 text-white hover:bg-white hover:text-surface-dark uppercase font-bold tracking-wide flex items-center gap-2"
              >
                <Link to="/resources">
          ...
          <div className="hidden md:block w-1/3">
            <div className="aspect-square bg-surface/10 border border-white/10 rounded-lg overflow-hidden backdrop-blur-sm p-4">
              <div
                className="w-full h-full border border-white/20 rounded relative"
                style={{
        ...
        <div className="container-page py-16 grid lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8 space-y-16">
                  ...
                  <table className="w-full text-sm text-left border border-border">
                    ...
                    <tbody>
                      {content.propertiesTable.rows.map((row, i) => (
                        <tr
                          key={i}
                          className="bg-background border-b border-border hover:bg-surface/50"
                        >
                          {row.map((cell, j) => (
                            <td
                              key={j}
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {content.types.map((type, i) => (
                    <div
                      key={i}
                      className="p-5 border border-border rounded relative group overflow-hidden bg-surface"
                    >
                      ...
                      <h3 className="font-bold text-sm uppercase tracking-wide mb-2 pl-2 group-hover:pl-4 transition-all">
                        {type.name}
                      </h3>
                      <p className="text-sm text-muted-foreground pl-2 group-hover:pl-4 transition-all">
                        {type.description}
                      </p>
                    </div>
                  ))}
            ...
            {content.faqs && (
              <div>
                <h2 className="font-display text-xl font-bold uppercase mb-6 text-foreground flex items-center gap-3">
                  <span className="text-primary">|</span> Frequently Asked Questions about{" "}
                  {content.label}
                </h2>
            ...
            {content.installationSpecs && (
              <div>
                <h2 className="font-display text-xl font-bold uppercase mb-4 text-foreground flex items-center gap-3">
                  <span className="text-primary">|</span> Specifications for Installing{" "}
                  {content.label}
                </h2>
            ...
            {content.projectReferences && (
              <div>
                <h2 className="font-display text-xl font-bold uppercase mb-6 text-foreground flex items-center gap-3">
                  <span className="text-primary">|</span> Projects with {content.label} Across
                  Africa
                </h2>
                <div className="grid sm:grid-cols-2 gap-4">
                  {content.projectReferences.map((project, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className="group relative h-40 rounded overflow-hidden cursor-pointer"
                      >
                        {cardContent}
                      </div>
                    );
            ...
            {content.popularProducts && content.popularProducts.length > 0 && (
              <div>
                ...
                <p className="text-sm text-muted-foreground mb-6">
                  A curated selection of best-selling {content.label.toLowerCase()}. View full
                  specs, request a quote, or add to your BOQ — without leaving this page.
                </p>
                <div className="grid sm:grid-cols-2 gap-4">
                  {content.popularProducts.map((p, i) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-bold text-sm uppercase tracking-wide text-foreground group-hover:text-primary transition">
                          {p.name}
                        </h3>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {content.industries.map((ind, i) => (
                    ...
                    >
                      ...
                      <span className="text-sm font-bold uppercase tracking-wide group-hover:text-primary transition">
                        {ind.label}
                      </span>
                    </Link>
                  ))}
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-6">
                Download technical datasheets and step-by-step installation guides for{" "}
                {content.label}.
              </p>
              <div className="grid sm:grid-cols-2 gap-4">
                <Link
                  to="/resources"
                  className="group flex items-center gap-4 border border-border bg-surface rounded p-5 hover:border-primary transition"
                >
                  ...
                  <div className="flex-1">
                    <div className="font-bold text-sm uppercase tracking-wide group-hover:text-primary transition">
                      Technical Datasheets
                    </div>
                    <div className="text-xs text-muted-foreground mt-0.5">
                      TDS, SDS & test certificates
                    </div>
                  </div>
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="rounded bg-surface-dark text-white p-8">
                ...
                <p className="text-sm text-white/70 mb-6 leading-relaxed">
                  Start your project with the right materials. Contact our sales team for
                  competitive pricing and availability across Africa.
                </p>
                <Button
                  asChild
                  className="w-full bg-primary hover:bg-primary-hover uppercase font-bold tracking-wide text-white"
                >
              ...
              <div className="rounded border border-border bg-surface p-6">
                <h3 className="font-display text-sm font-bold uppercase tracking-widest text-muted-foreground mb-4 border-b border-border pb-3">
                  Quick Links
                </h3>
        ...
        <div className="container-page py-16 flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            <h2 className="font-display text-3xl md:text-4xl font-bold uppercase tracking-tight text-white mb-2">
              Have a project? Partner with us.
            </h2>
            <p className="text-white/90 text-lg max-w-2xl">
              Get expert engineering support, reliable material supply, and best-in-class
              installation for your next geosynthetic containment project.
            </p>
          </div>
          <Button
            asChild
            size="lg"
            variant="outline"
            className="bg-transparent border-white text-white hover:bg-white hover:text-primary uppercase font-bold tracking-wider shrink-0"
          >
